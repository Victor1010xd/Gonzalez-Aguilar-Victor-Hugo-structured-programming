...
def gestionar_base_datos():
    try:
        ...
        if conexion.is_connected():
            ...
            # --- Menu de Inicio de 3 Opciones ---
            print("--- MENU PRINCIPAL ---")
            print("1. Registrar nuevo usuario y flor")
            print("2. Actualizar datos de un usuario")
            ...
            if opcion == "1":
                ...
                # --- Opcion Actualizar ---
                print("\n--- Actualizar Usuario ---")
                id_usuario_actualizar = int(input("Introduce el ID del usuario que deseas modificar: "))
                
                nuevo_nombre = input("Introduce el nuevo nombre de usuario: ")
                nuevo_email = input("Introduce el nuevo email: ")
                nuevo_password = input("Introduce la nueva contrasenia: ")
                ...
                WHERE id_usuarios = %s
                """
                cursor.execute(sql_actualizar, (nuevo_nombre, nuevo_email, nuevo_password, id_usuario_actualizar))
                conexion.commit()
                print(f"Usuario con ID {id_usuario_actualizar} actualizado con exito")
